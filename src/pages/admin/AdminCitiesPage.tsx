...
import type { AdminCity } from '@/types/api'

type CityForm = { name: string; branch: number; requires_deposit: boolean; is_active: boolean }

export function AdminCitiesPage() {
  const { t } = useTranslation()
  const [editCity, setEditCity] = useState<AdminCity | null>(null)
  const [showCreate, setShowCreate] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState<AdminCity | null>(null)
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="rounded-xl border border-border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <th className="px-4 py-3 text-start font-medium text-muted-foreground">{t('admin.cities.name')}</th>
                <th className="px-4 py-3 text-start font-medium text-muted-foreground hidden sm:table-cell">{t('admin.cities.branch')}</th>
                <th className="px-4 py-3 text-start font-medium text-muted-foreground">{t('admin.cities.requiresDeposit')}</th>
                <th className="px-4 py-3 text-start font-medium text-muted-foreground hidden md:table-cell">{t('admin.cities.active')}</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((c) => (
                <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground">{c.branch_name ?? c.branch}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${c.requires_deposit ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                      {c.requires_deposit ? t('common.yes') : t('common.no')}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${c.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
                      {c.is_active ? t('admin.cities.active') : t('branches.inactive')}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => setEditCity(c)}>
      ...
      {(showCreate || editCity) && (
        <CityFormDialog
          city={editCity}
          onClose={() => { setShowCreate(false); setEditCity(null) }}
        />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-background p-6 shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold">{t('admin.cities.confirmDelete')}</h2>
            ...
            <div className="flex gap-2">
              ...
              >
                {deleteCity.isPending ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}
                {t('common.delete')}
              </Button>
              <Button
                variant="outline"
...
function CityFormDialog({ city, onClose }: { city: AdminCity | null; onClose: () => void }) {
  const { t } = useTranslation()
  const { data: branches } = useAdminBranches()
  const create = useCreateAdminCity()
  const update = useUpdateAdminCity(city?.id ?? 0)
  const isEdit = !!city

  const { register, handleSubmit } = useForm<CityForm>({
    defaultValues: {
      name: city?.name ?? '',
      branch: city?.branch ?? 0,
      requires_deposit: city?.requires_deposit ?? false,
      is_active: city?.is_active ?? true,
    },
  })

  const onSubmit = async (values: CityForm) => {
    try {
      if (isEdit) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-background p-6 shadow-xl space-y-4">
        <h2 className="text-lg font-semibold">
          {isEdit ? t('admin.cities.editTitle') : t('admin.cities.addTitle')}
        </h2>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <Field label={t('admin.cities.name')}>
            <Input {...register('name')} required />
          </Field>
          <Field label={t('admin.cities.branch')}>
            <select
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" {...register('requires_deposit')} className="size-4" />
              {t('admin.cities.requiresDeposit')}
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" {...register('is_active')} className="size-4" />
              {t('admin.cities.active')}
            </label>
          </div>
